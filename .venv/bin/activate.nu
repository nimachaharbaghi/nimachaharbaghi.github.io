...
export-env {
    ...
    let virtual_env = r#'/home/runner/work/nimachaharbaghi.github.io/nimachaharbaghi.github.io/.venv'#
    let bin = r#'bin'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'personal portfolio-py3.10'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'personal portfolio-py3.10'#
    }
